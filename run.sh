...
if [ -e "$ledgerDir/genesis.bin" ] || [ -e "$ledgerDir/genesis.tar.bz2" ]; then
  ...
  fetch_program() {
    ...
    so="$download_path/spl_$name-$version.so"

    if [ "$loader" = "$upgradeableLoader" ]; then
      genesis_args="$genesis_args --upgradeable-program $address $loader $so none"
    else
      genesis_args="$genesis_args --bpf-program $address $loader $so"
    fi
